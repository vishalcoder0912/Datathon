    ...
    <Card className="border-slate-200">
      <CardContent className="flex flex-col p-0">
        ...
        <div className="border-t border-slate-200 p-4">
          <div className="flex gap-2"><input value={input} onChange={(event) => setInput(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); void handleSend(input); } }} placeholder="Ask KAVACH Copilot a supported question..." className="min-w-0 flex-1 rounded-lg border border-slate-200 bg-white px-4 py-2.5 text-sm outline-none placeholder:text-slate-400 focus:border-[#1D4ED8]" /><Button onClick={() => void handleSend(input)} disabled={loading || !input.trim()} className="gap-2 bg-[#1D4ED8]"><Send className="size-4" />Send</Button></div>
